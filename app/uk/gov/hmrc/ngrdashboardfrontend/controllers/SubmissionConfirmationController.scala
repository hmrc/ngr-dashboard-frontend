...
import uk.gov.hmrc.ngrdashboardfrontend.views.html.SubmissionConfirmationView
...
class SubmissionConfirmationController @Inject()(
  ...
  def onPageLoad(propertyReference: String): Action[AnyContent] =
    (authenticate andThen isRegisteredCheck).async( implicit request =>
      ngrService.linkedPropertyStatus(request.credId, request.nino).flatMap {
        case Some(property) => Future.successful(Ok(view(property.vmvProperty.addressFull, propertyReference, createDefaultNavBar)))
