...
trait ControllerSpecSupport extends TestSupport {
  ...
  def mockRequest(hasNino: Boolean = true): Unit =
    when(mockAuthJourney andThen mockIsRegisteredCheck) thenReturn new ActionBuilder[AuthenticatedUserRequest, AnyContent] {
      override def invokeBlock[A](request: Request[A], block: AuthenticatedUserRequest[A] => Future[Result]): Future[Result] =  {
        val authRequest = AuthenticatedUserRequest(request, None, None, Some("user@email.com"), CredId("1234"), Some(Name(Some("john"), Some("smith"))), None, nino = if (hasNino) Nino(hasNino = true, Some("AA000003D")) else Nino(hasNino = false, None))
        block(authRequest)
      }
  ...
  def mockLinkedPropertiesRequest(hasNino: Boolean = true): Unit = {
